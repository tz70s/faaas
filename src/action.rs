...
static UDS_SOCKET_PATH: &'static str = "actions/node-js-v8-uds/action.sock";
...
pub fn get_uds_endpoint(req: Request, handle: Handle) -> Box<Future<Item=Response, Error=hyper::Error>> {
    // FIXME: CRITICAL POINT HERE, EXTREMELY SLOW NOW!
    let id = &req.path()[14..];
    // FIXME: Hard coded param.
    ...
    let uds_stream = if let Ok(stream) = UnixStream::connect(UDS_SOCKET_PATH, &handle) {
        ...
        return Box::new(futures::future::ok(Response::new().with_status(StatusCode::BadRequest)));
    };

    Box::new(write_all(uds_stream, runtime_signal).and_then(|(uds_stream, mut buf)| {
        buf.clear();
